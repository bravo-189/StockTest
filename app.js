  ...
  function renderMarketOverview() {
    ...
    if (leaderRoot) {
      ...
      [...sectors.map((row) => ({ ...row, scope: "板块" })), ...industries.map((row) => ({ ...row, scope: "行业" }))]
        ...
        .filter((row) => row.daily && row.daily.delta >= RSI_GAIN_THRESHOLD)
        .forEach((row) => {
          const current = leadersByTicker.get(row.ticker);
          if (!current || row.daily.delta > current.daily.delta || (row.daily.delta === current.daily.delta && row.scope === "板块")) leadersByTicker.set(row.ticker, row);
        });
      const leaders = [...leadersByTicker.values()].sort((a, b) => b.daily.delta - a.daily.delta || a.ticker.localeCompare(b.ticker)).slice(0, 3);
      leaderRoot.innerHTML = leaders.length ? `<div class="market-rsi-leader-copy"><small>AFTER-HOURS RSI LEADERS</small><strong>盘后 RSI 强势标的</strong><span class="market-rsi-leader-detail">当日收盘后 RSI14 增加至少 ${RSI_GAIN_THRESHOLD} 点 · 按增幅取前三 · 同一代码仅保留一次</span></div><div class="market-rsi-leader-list">${leaders.map((leader, index) => `<a class="market-rsi-leader-item tradingview-link" href="${tradingViewUrl(leader.ticker)}" target="_blank" rel="noopener noreferrer" aria-label="在 TradingView 查看 ${leader.ticker}"><span class="market-rsi-leader-rank">TOP ${index + 1}</span><span class="market-rsi-leader-symbol">${html(leader.ticker)}</span><span class="market-rsi-leader-name">${html(leader.scope)} · ${html(leader.name)}</span><strong><small>Δ1 RSI</small>+${leader.daily.delta.toFixed(1)}</strong></a>`).join("")}</div>` : `<div class="market-rsi-leader-copy"><small>AFTER-HOURS RSI LEADERS</small><strong>盘后 RSI 强势标的</strong><span class="market-rsi-leader-detail">当日暂无 RSI14 增加至少 ${RSI_GAIN_THRESHOLD} 点的板块或行业 ETF</span></div>`;
    }
