(function () {
  ...
  function renderIndices() {
    ...
    root.innerHTML = indexDefs.map(([ticker, name, fallbackPrice, fallbackChange], index) => {
      ...
      const mode = ticker === "BTC" ? "当日 · 2H（24H）K 线" : "当日 · 5M 面积图"; const note = incomplete ? "未收盘 · 悬停放大近 1 个月" : "悬停放大近 1 个月"; const aria = `${name} ${ticker === "BTC" ? "当日 2 小时 K 线" : "当日 5 分钟折线面积图"}${incomplete ? "（含未收盘柱）" : ""}`;
  ...
  function renderSectors() {
    ...
    body.innerHTML = rows.length ? rows.map((row, index) => { const bars = marketBars[row.ticker]; const d1Delta = closeDelta(bars, 1); const d5Delta = closeDelta(bars, 5); const d20Delta = closeDelta(bars, 20); return `<tr class="${row.ticker === "SPY" ? "is-pinned" : ""}"><td class="rank">${String(index + 1).padStart(2, "0")}</td><td><a class="etf-button tradingview-link" href="${tradingViewUrl(row.ticker)}" target="_blank" rel="noopener noreferrer" aria-label="在 TradingView 查看 ${row.ticker}">${row.ticker}</a>${row.ticker === "SPY" ? '<span class="sub-label">锁定</span>' : ""}</td><td>${row.name}</td><td>${row.rsi.toFixed(1)}</td><td class="${classFor(d1Delta)}" title="绝对价格变化，不含百分号">${signedDelta(d1Delta)}</td><td class="${classFor(d5Delta)}" title="绝对价格变化，不含百分号">${signedDelta(d5Delta)}</td><td class="${classFor(d20Delta)}" title="绝对价格变化，不含百分号">${signedDelta(d20Delta)}</td><td><button class="holding-link" type="button" data-etf="${row.ticker}">前十大 →</button></td></tr>`; }).join("") : `<tr><td colspan="8"><div class="drawer-empty">没有匹配的板块或 ETF</div></td></tr>`;
