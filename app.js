(function () {
  ...
  function renderMarketOverview() {
    ...
    if (leaderRoot) {
      const sourceRows = [...sectors.map((row) => ({ ...row, scope: "板块" })), ...industries.map((row) => ({ ...row, scope: "行业" }))]
        .map((row) => ({ ...row, daily: rsiDailyChangeFor(row.ticker) }))
      const uniqueRows = (rows, isStronger) => {
        const leadersByTicker = new Map();
        rows.forEach((row) => {
          const current = leadersByTicker.get(row.ticker);
          if (!current || isStronger(row, current) || (!isStronger(current, row) && row.scope === "板块")) leadersByTicker.set(row.ticker, row);
        });
        return [...leadersByTicker.values()];
      };
      const dailyLeaders = uniqueRows(sourceRows.filter((row) => row.daily && row.daily.delta >= RSI_GAIN_THRESHOLD), (row, current) => row.daily.delta > current.daily.delta)
        .sort((a, b) => b.daily.delta - a.daily.delta || a.ticker.localeCompare(b.ticker));
      const useRsiFallback = !dailyLeaders.length;
      const leaders = (useRsiFallback
        ? uniqueRows(sourceRows.filter((row) => Number.isFinite(Number(row.rsi))), (row, current) => Number(row.rsi) > Number(current.rsi)).sort((a, b) => Number(b.rsi) - Number(a.rsi) || a.ticker.localeCompare(b.ticker))
        : dailyLeaders).slice(0, 6);
      const detail = useRsiFallback ? "当日暂无 RSI14 增加至少 " + RSI_GAIN_THRESHOLD + " 点的标的 · 已按当前 RSI14 最高展示" : "当日收盘后 RSI14 增加至少 " + RSI_GAIN_THRESHOLD + " 点 · 按增幅展示";
      leaderRoot.innerHTML = leaders.length ? `<div class="market-rsi-leader-copy"><small>AFTER-HOURS RSI LEADERS</small><strong>盘后 RSI 强势标的</strong><span class="market-rsi-leader-detail">${detail} · 前六 · 同一代码仅保留一次</span></div><div class="market-rsi-leader-list">${leaders.map((leader, index) => `<a class="market-rsi-leader-item tradingview-link" href="${tradingViewUrl(leader.ticker)}" target="_blank" rel="noopener noreferrer" aria-label="在 TradingView 查看 ${leader.ticker}"><span class="market-rsi-leader-rank">TOP ${index + 1}</span><span class="market-rsi-leader-symbol">${html(leader.ticker)}</span><span class="market-rsi-leader-name">${html(leader.scope)} · ${html(leader.name)}</span><strong><small>${useRsiFallback ? "RSI14" : "Δ1 RSI"}</small>${useRsiFallback ? Number(leader.rsi).toFixed(1) : "+" + leader.daily.delta.toFixed(1)}</strong></a>`).join("")}</div>` : `<div class="market-rsi-leader-copy"><small>AFTER-HOURS RSI LEADERS</small><strong>盘后 RSI 强势标的</strong><span class="market-rsi-leader-detail">暂无可用的板块或行业 RSI 数据</span></div>`;
    }
  ...
  function renderPositionCalculator() {
    ...
    const stopValid = Number.isFinite(price) && price > 0 && Number.isFinite(stop) && stop >= 0 && stop < price; const perShareRisk = stopValid ? price - stop : null; const stopPercent = stopValid ? ((price - stop) / price) * 100 : null; const riskShares = stopValid && Number.isFinite(fixedRisk) ? Math.floor(fixedRisk / perShareRisk) : null;
    ...
    const stopPercentLabel = Number.isFinite(stopPercent) ? `${stopPercent.toFixed(2).replace(/0+$/, "").replace(/\.$/, "")}%` : "5%"; const stopDisplay = stopValid ? `${formatUsd(stop)}（下跌 ${stopPercentLabel}）` : (stopInput.dataset.autoStop !== "false" ? "默认下跌 5%" : "未设置");
    $("#calc-fixed-risk").textContent = formatUsd(fixedRisk); $("#calc-fixed-risk-rate").textContent = `总资金 × ${riskRateLabel}`; $("#calc-result-funds").textContent = formatUsd(funds); $("#calc-result-risk").textContent = formatUsd(fixedRisk); $("#calc-result-price").textContent = formatUsd(price); $("#calc-result-planned").textContent = formatUsd(planned); $("#calc-result-base-shares").textContent = Number.isFinite(baseShares) ? `${baseShares} 股` : "—"; $("#calc-result-used").textContent = formatUsd(actualUsed); $("#calc-result-stop").textContent = stopDisplay; $("#calc-result-per-share-risk").textContent = stopValid ? formatUsd(perShareRisk) : "未触发"; $("#calc-result-final-shares").textContent = Number.isFinite(finalShares) ? `${finalShares} 股` : "—"; $("#calc-selected-percent").textContent = `已选 ${(percent * 100).toFixed(0)}%`;
