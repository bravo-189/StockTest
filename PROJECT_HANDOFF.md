...
**当前阶段：** 网页原型、本地数据质量、异常状态与自动刷新发布链路均已完成；板块/行业每日 RSI 历史表、SPY 滚动固定、前十大持仓名单和指数 TradingView 跳转已完成；当前收束为“只展示前十大名单、不展示持仓比例、保留来源异常证据”。数据契约见 [docs/DATA_PIPELINE.md](./docs/DATA_PIPELINE.md)；主题 AI 排名和小程序仍暂缓。
...
3. TightPlayer 实际完整域名已从截图确认并写入 Pages：`www.tightplayer.asia`。当前只等待 DNS 生效。
4. 阿里云页面已确认 CNAME 正确且启用；当前要检查的是 `.asia` 域名权威 NS 委派。页面列出的阿里云 NS 为 `dns13.hichina.com`、`dns14.hichina.com`。如域名当天注册/实名认证，先等待次日同步；NS 生效后再验证 HTTPS、重定向和线上资源路径，再开启 HTTPS 强制。
